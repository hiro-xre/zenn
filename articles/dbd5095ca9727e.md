...
## テンプレート部を簡潔にする
テンプレート（`<template>`）部に記すロジックは最低限にします。
...
テンプレート部に書いていた `value1 === 0 && value2 === 0` という条件を、`isDisplayText` という `computed` に置き換えています。こうすることで以下の利点があります。
