...
# おまけ（個人的スタイルガイド）
以下は個人的に意識しているものの中から、効果的かつ利用頻度の高いものを記載しています。
是非参考にしてみてください。

## テンプレート内を簡潔にする
テンプレート（`<template>`）内に記すロジックは最低限にします。
これは、UI 宣言（`<template>`）と処理（`<script>`）の関心を明確にわける意図があります。
例えば以下のコードがあるとします。
```ts
...
const value1 = ref(0);
const value2 = ref(0);
const isDisplayText = computed<boolean>(() => {
...
<template>
  <div v-if="isDisplayText">
    条件によって表示・非表示を分けます。
  </div>
</template>
```

テンプレート部に書いていた `value1 === 0 && value2 === 0` という条件を、`isDisplayText` という `computed` に置き換えています。こうすることで以下の利点がありますので、試してみてください。
* テンプレートが簡潔になり、DOM 構造を視覚的に把握しやすい
* 条件を変数として命名することで、どのような条件なのかわかりやすい
* 条件が肥大化しても、テンプレートが肥大化しない
